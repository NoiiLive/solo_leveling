...
public record HunterSkillGUIMageWaterButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<HunterSkillGUIMageWaterButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(SlsbMod.MODID, "hunter_skill_gui_mage_water_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, HunterSkillGUIMageWaterButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, HunterSkillGUIMageWaterButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new HunterSkillGUIMageWaterButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<HunterSkillGUIMageWaterButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final HunterSkillGUIMageWaterButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HunterSkillGUIMageWaterMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SlsbMod.addNetworkMessage(HunterSkillGUIMageWaterButtonMessage.TYPE, HunterSkillGUIMageWaterButtonMessage.STREAM_CODEC, HunterSkillGUIMageWaterButtonMessage::handleData);
	}
}
