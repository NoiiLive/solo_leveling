...
public record HunterSkillGUIFighterButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<HunterSkillGUIFighterButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(SlsbMod.MODID, "hunter_skill_gui_fighter_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, HunterSkillGUIFighterButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, HunterSkillGUIFighterButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new HunterSkillGUIFighterButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<HunterSkillGUIFighterButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final HunterSkillGUIFighterButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HunterSkillGUIFighterMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SlsbMod.addNetworkMessage(HunterSkillGUIFighterButtonMessage.TYPE, HunterSkillGUIFighterButtonMessage.STREAM_CODEC, HunterSkillGUIFighterButtonMessage::handleData);
	}
}
