...
public record HunterSkillGUIMageWindButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<HunterSkillGUIMageWindButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(SlsbMod.MODID, "hunter_skill_gui_mage_wind_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, HunterSkillGUIMageWindButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, HunterSkillGUIMageWindButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new HunterSkillGUIMageWindButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<HunterSkillGUIMageWindButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final HunterSkillGUIMageWindButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HunterSkillGUIMageWindMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SlsbMod.addNetworkMessage(HunterSkillGUIMageWindButtonMessage.TYPE, HunterSkillGUIMageWindButtonMessage.STREAM_CODEC, HunterSkillGUIMageWindButtonMessage::handleData);
	}
}
