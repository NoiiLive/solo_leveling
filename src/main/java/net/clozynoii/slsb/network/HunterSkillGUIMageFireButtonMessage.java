...
public record HunterSkillGUIMageFireButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<HunterSkillGUIMageFireButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(SlsbMod.MODID, "hunter_skill_gui_mage_fire_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, HunterSkillGUIMageFireButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, HunterSkillGUIMageFireButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new HunterSkillGUIMageFireButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<HunterSkillGUIMageFireButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final HunterSkillGUIMageFireButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HunterSkillGUIMageFireMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SlsbMod.addNetworkMessage(HunterSkillGUIMageFireButtonMessage.TYPE, HunterSkillGUIMageFireButtonMessage.STREAM_CODEC, HunterSkillGUIMageFireButtonMessage::handleData);
	}
}
