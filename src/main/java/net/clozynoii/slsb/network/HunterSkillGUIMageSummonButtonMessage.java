...
public record HunterSkillGUIMageSummonButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<HunterSkillGUIMageSummonButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(SlsbMod.MODID, "hunter_skill_gui_mage_summon_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, HunterSkillGUIMageSummonButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, HunterSkillGUIMageSummonButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new HunterSkillGUIMageSummonButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<HunterSkillGUIMageSummonButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final HunterSkillGUIMageSummonButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HunterSkillGUIMageSummonMenu.guistate;
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SlsbMod.addNetworkMessage(HunterSkillGUIMageSummonButtonMessage.TYPE, HunterSkillGUIMageSummonButtonMessage.STREAM_CODEC, HunterSkillGUIMageSummonButtonMessage::handleData);
	}
}
